...
func NewReleaseChannelConfigMap(kubernetesConfig *config.Kubernetes, settings settings.Settings) ReleaseChannelConfigMap {
	return ReleaseChannelConfigMap{
		KubernetesConfig: kubernetesConfig,
		Settings:         settings,
	}
}

type ReleaseChannelConfigMap struct {
	KubernetesConfig *config.Kubernetes
	Settings         settings.Settings
}

func (s ReleaseChannelConfigMap) Create(ctx context.Context, releaseChannel types.ReleaseChannel) error {
	...
	cm := v1.ConfigMap{
		...
		Data: releaseChannel.ToConfigMapData(),
	}
...
func (s ReleaseChannelConfigMap) Get(ctx context.Context) (*types.ReleaseChannel, error) {
	c := s.KubernetesConfig.Client()
	...
	// todo: configmap to types.releaseChannel
	return &types.ReleaseChannel{}, nil
}

func (s ReleaseChannelConfigMap) Update(ctx context.Context, releaseChannel types.ReleaseChannel) error {
	c := s.KubernetesConfig.Client()
	...
	cm.Data = releaseChannel.ToConfigMapData()
